...
temp_df <- as.data.frame(temp, xy = TRUE, cells = FALSE, na.rm = TRUE)
names(temp_df) <- c('x','y','temp_c')
write.csv(temp_df, file.path(out_dir, 'surface_temperature_map.csv'), row.names = FALSE)
